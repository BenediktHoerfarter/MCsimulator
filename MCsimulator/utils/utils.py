...
def ljEnergyCalculator(atomsWithCoordinatesList: list, parameterString: str, vdWCutoff: float) -> float:
    EpotTotal: float = 0.0
    numAtoms: int = len(atomsWithCoordinatesList)

    for i in range(numAtoms - 1):
        atomA = atomsWithCoordinatesList[i]
        ...
        for j in range(i + 1, numAtoms):
            ...
            if squareDistance <= vdWCutoff**2:
                ...
                Blj: float = float(parameterString[BljPosStart + 1 : BljPosEnd].strip())
